...
export function ExternalLinkLogoButton({ type, href, disabledReason, onClick, size = "sm", ariaLabel }: Props) {
  const cfg = CONFIG[type];
  const disabled = !href || !!disabledReason;
  const title = disabled ? disabledReason ?? cfg.tooltip : cfg.tooltip;
  ...
  return (
    <a href={href!} target="_blank" rel="noopener noreferrer" onClick={onClick} title={title}
      aria-label={ariaLabel ?? cfg.tooltip}
