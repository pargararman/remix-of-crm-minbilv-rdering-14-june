...
export type DealerAccount = {
  ...
  hasAccount: boolean;
  users: { userId: string; email: string | null; lastSignInAt: string | null }[];
};

export const listAccountOverview = createServerFn({ method: "GET" })
  .middleware([requireAdmin])
  .handler(async () => {
    const [{ data: profiles }, { data: dealers }, { data: dealerUsers }] = await Promise.all([
      supabaseAdmin
        .from("profiles")
    ...
    const usersByDealer = new Map<string, { userId: string; email: string | null; lastSignInAt: string | null }[]>();
