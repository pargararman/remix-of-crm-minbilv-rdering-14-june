...
src/components/leads/lead-overview-header.tsx      # header Blocket btn -> API + persists pricing
```

> Both Blocket buttons on the lead profile (the header AND the Snabb värdering
> panel) now run the API. The header has no Save bar, so its "Använd i
> prissättning" persists immediately via `updatePricing` and invalidates the
> `["pricing", leadId]` query. The Blocket button in the leads LIST
> (`lead-card.tsx`) is intentionally left as a link.
